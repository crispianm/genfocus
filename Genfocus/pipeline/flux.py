...
def attn_forward(
    ...
    **kwargs: dict,
    ...
    cache_idx = getattr(attn, "cache_idx", None)

    if cache_mode == "write" and cache_idx is not None:
        for i, (k, v) in enumerate(zip(keys, values)):
            if to_cache[i]:
                cache_storage[cache_idx][0].append(k)
                cache_storage[cache_idx][1].append(v)
    ...
    for i, query in enumerate(queries):
        ...
        for j, (k, v) in enumerate(zip(keys, values)):
            ...
            keys_.append(k)
            values_.append(v)
        if cache_mode == "read" and cache_idx is not None:
            keys_.extend(cache_storage[cache_idx][0])
            values_.extend(cache_storage[cache_idx][1])
...
# Determine if we need tiled denoising
    ...
    if kv_cache:
        # diffusers' Flux attention class name/type can vary across versions.
        # We use duck-typing instead of strict isinstance(Attention).
        def _is_attn_like(m: object) -> bool:
            return (
                hasattr(m, "to_q")
                and hasattr(m, "to_k")
                and hasattr(m, "to_v")
                and hasattr(m, "heads")
                and hasattr(m, "norm_q")
                and hasattr(m, "norm_k")
            )

        attn_modules = [m for m in self.transformer.modules() if _is_attn_like(m)]
        for idx, module in enumerate(attn_modules):
            setattr(module, "cache_idx", idx)
