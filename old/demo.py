...
def preprocess_input_image(raw_img, target_long_side):
    if raw_img is None:
        return None, None, None, None, None, "Center (Default)"
    ...
    # Return 5 items + 1 label
    return final_input, final_input, None, None, None, "Center (Default)"
...
def get_intermediate_results(clean_input_processed, disable_tiling_tricks_for_accel, num_inference_steps, intermediate_cache):
    if intermediate_cache is not None:
        print("✅ Using cached Deblur/Depth results.")
        return intermediate_cache['deblurred_img'], intermediate_cache['disp']
    ...
    print(f"🚀 Running Stage 1: DeblurNet (Size: {w}x{h})")
    switch_lora("deblur")
    ...
    print("   ► Running Stage 2: Depth Pro")
    try:
    ...
    return deblurred_img, disp

def run_genfocus_pipeline(clean_input_processed, click_coords, K_value, cached_latents, disable_tiling_tricks_for_accel, num_inference_steps, intermediate_cache):
    ...
    deblurred_img, disp = get_intermediate_results(clean_input_processed, disable_tiling_tricks_for_accel, num_inference_steps, intermediate_cache)
    new_cache = {'deblurred_img': deblurred_img, 'disp': disp}

    if K_value == 0:
        print("✅ K=0, returning Deblur result.")
        return deblurred_img, cached_latents, new_cache
    ...
    return generated_bokeh, current_latents, new_cache

def run_focal_sweep(clean_input_processed, K_value, num_frames, disable_tiling_tricks_for_accel, num_inference_steps, intermediate_cache):
    if clean_input_processed is None:
    ...
    output_dir = os.path.join(output_base, f"sweep_{int(time.time())}")
    os.makedirs(output_dir, exist_ok=True)
    
    disp_vals = disp.flatten()
    min_d = np.percentile(disp_vals, 1)
    max_d = np.percentile(disp_vals, 99)
    focus_values = np.linspace(min_d, max_d, int(num_frames))
    
    force_no_tile = min(w, h) < 512
    no_tiled_denoise = bool(disable_tiling_tricks_for_accel) or force_no_tile
    
    saved_paths = []
    print(f"🚀 Starting Focal Sweep: {num_frames} frames. Output: {output_dir}")
    
    for i, fd in enumerate(focus_values):
        print(f"   [{i+1}/{num_frames}] Focusing on disp={fd:.4f}")
    ...
    return f"✅ Saved {len(saved_paths)} images to: {output_dir}", new_cache
...
with gr.Blocks(css=css) as demo:
    ...
    intermediate_cache_state = gr.State(value=None)
        ...
        * **Refocusing:** 1. **Click** on the subject you want to bring into focus in the **Step 2** image preview.  
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("🎥 Generate Focal Sweep (Datapoint)", open=False):
                    gr.Markdown("Generate a series of images focusing at different depths across the image.")
                    with gr.Row():
                        num_frames_slider = gr.Slider(minimum=2, maximum=100, value=10, step=1, label="Number of Frames")
                        sweep_btn = gr.Button("🎬 Generate Sweep", scale=1)
        ...
        input_raw.change(
            ...
            outputs=[focus_preview_img, clean_processed_state, latents_state, click_coords_state, intermediate_cache_state, click_status],
        )
        input_raw.upload(
            fn=preprocess_input_image,
            inputs=[input_raw, target_long_side_num],
        ...
        run_btn.click(
            ...
            inputs=[
                ...
                steps_slider,
                intermediate_cache_state,
            ],
            outputs=[output_img, latents_state, intermediate_cache_state],
        )
        ...
        sweep_btn.click(
            ...
            outputs=[sweep_status, intermediate_cache_state]
        )
